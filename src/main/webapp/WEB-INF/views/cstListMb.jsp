...
<html>
	...
	<body>
        <jsp:include page="./commons/loginBox.jsp"></jsp:include>
            <h3>상담신청 내역 리스트</h3>
            <br/>
            <h5>미열람 갯수 :  ${sessionScope.mbCnt}개</h5>
...
<!-- <button onclick="cstHidden()">삭제하기</button> -->
	   ...
	   <table>
	      <thead>
	         <tr>
				<th width="2px"><input type="checkbox" id ="all"/></th>
				<th width="5px">번호</th>
	            <th width="50px">교육기관명</th>
	            <th width="250px">교육과정명</th>
	            <th width="20px">신청일</th>
	            <th width="20px">열람여부</th>
	         </tr>
	      </thead>
	...
	function drawList(mbList){
		
	    var content = '';
	    
	    mbList.forEach(function(item){
	    	if(item.cons_delete_mb == false){
	...
	function cstDel(){
		...
		$('#mbList input[type="checkbox"]:checked').each(function(cons_idx,item){
