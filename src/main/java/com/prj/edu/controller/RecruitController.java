...
public class RecruitController {
	...
	public String recruitDetail(Model model, @RequestParam String idx, HttpSession session) {
...
//		logger.info("content : " + dto.getRecruit_content());
	...
	public String update(MultipartFile[] r_photo, @RequestParam HashMap<String, String> params, HttpServletRequest session) {
		 
		logger.info("수정 요청");
		logger.info("param : {}",params);
		
		// 수정하기 기능
		service.update(r_photo, params);
		
		// 수정을 완료하면 수정된 게시글을 바로 확인할 수 있도록 recruit_idx(모집공고 번호)를 가지고 상세보기로 이동
		return "redirect:/recruit/detail.do?idx=" + params.get("recruit_idx");
	}
